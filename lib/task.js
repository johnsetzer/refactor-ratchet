...
Task.prototype.src = function (options, srcCallback) {
  ...
  .spread(function (changedFiles) {
    ...
    console.log('BEFORE', changedFiles, this.gitTrackedFiles);
    this.mostRecentFiles = _.filter(this.mostRecentFiles, function (cf) {
      return _.include(this.gitTrackedFiles, cf.filePath);
    });
    
    console.log('AFTER', changedFiles);
...
Task.prototype.dest = function (destCb) {
  ...
  stream.on('end', function() {
    ...
    Promise.all(newFileMetrics).then(function() {
      // TODO query a fresh set of FileMetrics.mostRecentFiles() or do in process memory math to get new set
