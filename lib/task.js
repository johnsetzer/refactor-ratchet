...
var Task = function (options) {
  ...
  this.totalKeys = []; // Keys we want to calc min, max, median, mean, etc.. on.
...
Task.prototype.src = function (options) {
  // git tracked files
  // gitChanged.trackedFilesAsync('HEAD').then(function(files) {
  ...
  // gitChanged.lastCommitAsync().then(function(commit) {
  ...
  // query mostRecent
  // search for mostRecentCommitTime
  // mostRecent = intersection of most recent and tracked
  // changed = git diff mostRecentCommit..HEAD
  // filter changed for deleted files
  // filesToProcess = changed || untrackedByRR()


  this.filesToProcess = ['lib/console_reporter.js'];
