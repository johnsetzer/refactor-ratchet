...
var Task = function (options) {
  ...
  this.done = options.done || function (err, totalMetrics, commit, cb) { cb (); }
...
Task.prototype.helper = function () {
  ...
  return {
    ...
    getFileObj: function (file) {
      var filePath = typeof file === 'string' ? file : this.filePath(file);
...
Task.prototype.dest = function (destCb) {
  ...
  stream.on('end', function() {
    ...
    Promise.all(newFileMetricsPromises).then(function() {
      // Mix rrMostRecentFiles back into fileMetrics
      // before calculating total stats
      // Question:
      // Is this bad? Should I keep 
      // allFileMetrics(fileMetrics after to this step) and 
      // recentlyChangedMetrics(fileMetrics prior to this step) separate?
      _.each(task.rrMostRecentFiles, function (rf) {
        task.helper().setFileMetric(rf.filePath, rf.key, rf.value);
      });
      ...
      task.saveTotalMetrics().then(function () {
        task.done(null, task.totalMetrics, task.toCommit, destCb);
      }, function (err) {
        console.log(err);
      });
    }, function (err) {
